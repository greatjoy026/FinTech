...
export function requestIdMiddleware(req: Request, res: Response, next: NextFunction) {
  ...
  const requestId = supplied && REQUEST_ID_PATTERN.test(supplied) && supplied.length <= MAX_REQUEST_ID_LENGTH
    ? supplied
    : crypto.randomUUID();
...
export function errorHandler(error: unknown, _req: Request, res: Response, _next: NextFunction) {
  ...
  console.error('[HTTP] Unhandled request error', { requestId });
...
export function hasWildcardOrigin(headers: IncomingHttpHeaders): boolean {
