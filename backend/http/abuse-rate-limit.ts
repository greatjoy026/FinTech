...
function bucketKey(scope: string, identity: string): string {
  return `api:abuse:${scope}:${digest(identity)}`;
}
...
export type AbuseRateLimitOptions = {
  scope: string;
  limit: number;
  windowSeconds: number;
};

export async function checkAbuseRateLimit(
  scope: string,
  identity: string,
  limit: number,
  windowSeconds: number,
  ...
  try {
    ...
    // Production API abuse controls fail closed if the distributed limiter is unavailable.
...
export function abuseRateLimit(options: AbuseRateLimitOptions) {
  ...
  return async (req: Request, res: Response, next: NextFunction) => {
    ...
    if (!result.allowed) {
      ...
      return res.status(429).json({ error: 'Too many requests', requestId: res.getHeader('X-Request-ID') });
    }
    return next();
  };
}
