...
export async function checkAbuseRateLimit(scope: string, identity: string, limit: number, windowSeconds: number): Promise<{ allowed: boolean; retryAfter: number }> {
  ...
  catch { logger.error('ABUSE_LIMITER_UNAVAILABLE', { scope }); return { allowed: false, retryAfter: 1 }; }
}
function identityFor(req: Request): string { const principal = (req as Request & { user?: { userId?: string } }).user?.userId; return principal ? `user:${principal}` : `ip:${req.ip || 'unknown'}`; }
