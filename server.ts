...
async function startServer() {
  ...
  app.use(cors({ origin: (origin, callback) => { if (!origin || env.allowedOrigins.includes(origin)) return callback(null, true); return callback(new Error('CORS origin denied')); }, credentials: true, methods: ['GET', 'POST', 'PUT', 'PATCH', 'DELETE', 'OPTIONS'], allowedHeaders: ['Authorization', 'Content-Type', 'X-Request-ID'], maxAge: 600 }));
  ...
  try { QueueService.initializeProcessors(); logger.info('QUEUE_PROCESSORS_INITIALIZED'); } catch (error) { logger.error('QUEUE_PROCESSOR_INITIALIZATION_FAILURE', { error: error instanceof Error ? error.name : 'unknown' }); }
  ...
  app.get('/api/admin-only', authenticate, requireAdmin, requirePrivilegedOperation('PRODUCTION_ACCESS'), abuseRateLimit({ scope: 'admin-only', limit: 60, windowSeconds: 60 }), async (req: AuthRequest, res, next) => { try { if (!req.user) return res.status(401).json({ error: 'Unauthorized' }); await AuditService.record({ actorId: req.user.userId, role: req.user.role, action: 'ADMIN_API_ACCESS', resource: 'ADMIN_API', resourceId: req.path, outcome: 'SUCCESS', requestId: String(req.headers['x-request-id'] ?? ''), ipAddress: req.ip, device: String(req.headers['user-agent'] ?? '').slice(0, 512) || undefined }); return res.json({ message: 'Welcome Admin' }); } catch (error) { return next(error); } });
  if (process.env.NODE_ENV !== 'production') { const vite = await createViteServer({ server: { middlewareMode: true }, appType: 'spa' }); app.use(vite.middlewares); } else { const distPath = path.join(process.cwd(), 'dist'); app.use(express.static(distPath, { index: false })); app.get('*all', (req, res) => req.path.startsWith('/api/') ? notFoundHandler(req, res) : res.sendFile(path.join(distPath, 'index.html'))); }
